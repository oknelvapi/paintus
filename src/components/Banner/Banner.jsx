/* eslint-disable max-len */
import React, { memo } from 'react';
import PropTypes from 'prop-types';
import cx from 'classnames';

import { Box, Typography, SvgIcon } from '@material-ui/core';
import WorkOutlineOutlinedIcon from '@material-ui/icons/WorkOutlineOutlined';
import CropOriginalOutlinedIcon from '@material-ui/icons/CropOriginalOutlined';

import bannerStyles from './styles';

const Banner = memo(({ page, title, text, triangular }) => {
  const classes = bannerStyles();
  return (
    <Box id="banner" component="section" className={cx(classes.banner, { [classes.notTriangular]: !triangular })}>
      <Box className={classes.inner}>
        <Box id="logo" className={classes.logoWrap}>
          <SvgIcon className={classes.logo}>
            <Choose>
              <When condition={page === 'offers'}>
                <WorkOutlineOutlinedIcon />
              </When>
              <When condition={page === 'works'}>
                <CropOriginalOutlinedIcon />
              </When>
              <Otherwise>
                <>
                  <g><rect fill="none" height="24" width="24" /></g>
                  <g>
                    <g>
                      <rect height="8.48" transform="matrix(0.7071 -0.7071 0.7071 0.7071 -6.8717 17.6255)" width="3" x="16.34" y="12.87" />
                      <path d="M17.5,10c1.93,0,3.5-1.57,3.5-3.5c0-0.58-0.16-1.12-0.41-1.6l-2.7,2.7L16.4,6.11l2.7-2.7C18.62,3.16,18.08,3,17.5,3 C15.57,3,14,4.57,14,6.5c0,0.41,0.08,0.8,0.21,1.16l-1.85,1.85l-1.78-1.78l0.71-0.71L9.88,5.61L12,3.49 c-1.17-1.17-3.07-1.17-4.24,0L4.22,7.03l1.41,1.41H2.81L2.1,9.15l3.54,3.54l0.71-0.71V9.15l1.41,1.41l0.71-0.71l1.78,1.78 l-7.41,7.41l2.12,2.12L16.34,9.79C16.7,9.92,17.09,10,17.5,10z" />
                    </g>
                  </g>
                </>
              </Otherwise>
            </Choose>

          </SvgIcon>
        </Box>
        <Typography variant="h1" className={classes.title}>{title}</Typography>
        <Typography className={classes.subtitle}>{text}</Typography>
      </Box>
    </Box>
  );
});

Banner.propTypes = {
  triangular: PropTypes.bool,
  page: PropTypes.oneOf(['home', 'offers', 'works']).isRequired,
  title: PropTypes.string.isRequired,
  text: PropTypes.string.isRequired
};

Banner.defaultProps = {
  triangular: false,
};

export default Banner;
